...
while (<$INPUT>) {
	...
	while (<$TMPFILE>) {
		...
		if ($base_readpos) {
			...
			my @chrlist = split (/\_/, $chr);
                        		my $chrtmp = $chrlist[0];
                        		open (my $FAFILESEP, ">>", "$fafile.$chrtmp");
                        		print $FAFILESEP ">$chr\-$position\-$mismatchreadcount\n$sequence\n";
                        		close $FAFILESEP;
		}
	}
	close $TMPFILE;
...
#system("blat -stepSize=5 -repMatch=2253 -minScore=20 -minIdentity=0 -noHead $refGenome $fafile $pslfile");
#system("rm $fafile");

system("time parallel -j $ncore blat -stepSize=5 -repMatch=2253 -minScore=20 -minIdentity=0 -noHead $refGenome $fafile.chr{} $pslfile.chr{} ::: {1..22} X Y M Un");
system("cat $pslfile.chr* > $pslfile");
system("rm $fafile.chr* $pslfile.chr*");

#bowtie2 -p $ncore --local -a -f --rfg 0,0 --rdg 0,0 --score-min G,20,0 --no-hd -x ../ucsc.hg19 -U $fafile -S $samfile
...
while(<PSL>) { #parse PSL outputfile: for each mismatched read that was blatted, hash together all the blat hits 
	chomp;
	my @pslfields = split;
	my $name = $pslfields[9]; #qName: query sequence name chr_pos
	my $blatscore = join '@',$pslfields[0],$pslfields[13],$pslfields[17],$pslfields[18],$pslfields[20]; #matches, tName, blockCount, blockSizes, tStarts
	if ($pslhash{$name}) {
