#script that removes mismatches in first 7 bp of reads that are caused by imperfect priming while making cDNA from RNA
...
while (<$INPUT>) {
	...
	if ($newmismatch >= $minmismatch) { #only output candidates that have mismatched reads remaining
		...
		print $OUTPUT "$fields[0]\t$fields[1]\t$fields[2]\t$newcov,$newmismatch\t$fields[4]\t$fields[5]\t$varfreq";
